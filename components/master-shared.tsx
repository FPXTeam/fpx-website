...
export const pageData: Record<string,{eyebrow:string;title:string;intro:string}> = {
  ...
  "source-timber": {eyebrow:"SOURCE TIMBER THROUGH FPX",title:"Find the timber your project needs.",intro:"Explore current timber supply, review offers or submit a specific requirement before creating an account. Sign in or request access when you need the full customer experience."},
  "how-fpx-works": {eyebrow:"HOW FPX WORKS",title:"Three ways to start. One supported process.",intro:"Explore timber, review current offers or create a detailed request without an account, then move into the full FPX customer experience when you are ready."},
  ...
  "faq": {eyebrow:"FPX SOURCING FAQ",title:"Questions about sourcing timber through FPX.",intro:"Direct answers about exploring FPX without an account, current offers, product requests, full customer access, orders and support."},
  "frequently-asked-questions": {eyebrow:"FPX SOURCING FAQ",title:"Questions about sourcing timber through FPX.",intro:"Direct answers about exploring FPX without an account, current offers, product requests, full customer access, orders and support."},
...
export const AppButtons=()=> <div className="m-actions"><a className="m-btn m-btn-primary" href="https://app.fpx.nz/explore-fpx-sourcing">Browse timber <Arrow/></a><a className="m-btn m-btn-ghost" href="https://app.fpx.nz/explore-fpx-sourcing?request=1">Create a request <Arrow/></a></div>;
...
export function LightCTA(){return <section className="m-cta"><div className="m-cta-panel"><div className="m-cta-lines"/><div className="m-cta-content"><Eyebrow>START SOURCING</Eyebrow><h2>Ready to source <span className="headline-accent">smarter?</span></h2><p>Explore current timber or submit a requirement without creating an account.</p><AppButtons/></div><Image className="m-cta-x" src="/images/fpx-logo-x-original.png" alt="" width={495} height={504} sizes="(max-width: 700px) 45vw, 32vw" quality={80}/></div></section>;}
export function InnerHero({slug}:{slug:string}){const d=pageData[slug] ?? pageData["source-timber"];const image=slug==="our-customers"?"customer-buyers.webp":slug==="source-timber"?"category-manufacturing-grades.webp":slug==="about-us"?"fpx-hero-timber-yard.webp":"category-structural-timber.webp";const imageAlt=slug==="our-customers"?"New Zealand commercial timber buyers":slug==="source-timber"?"Commercial timber stock available for sourcing through FPX":slug==="about-us"?"New Zealand timber stored in a commercial yard":"Structural timber framing in New Zealand";return <section className={`m-inner-hero hero-${slug} m-animate-in`}><div><Eyebrow>{d.eyebrow}</Eyebrow><h1>{d.title}</h1><p>{d.intro}</p>{!["faq","saw-point","industry-resources","terms-and-conditions","privacy-policy","cookie-policy","cookie-settings"].includes(slug)&&<AppButtons/>}</div>{!['terms-and-conditions','privacy-policy','cookie-policy','cookie-settings','faq','saw-point','industry-resources','contact-us'].includes(slug)&&<div className="m-inner-visual"><img src={`/images/${image}`} alt={imageAlt}/>{slug!=="how-fpx-works"&&<span>{d.eyebrow}</span>}</div>}</section>;}
export function PageCTA(){return <LightCTA/>}
export function SourcePageCTA(){return <section className="m-cta m-source-cta"><div className="m-cta-panel"><div className="m-cta-lines"/><div className="m-cta-content"><Eyebrow>START SOURCING</Eyebrow><h2>Ready to source <span className="headline-accent">smarter?</span></h2><p>Explore current timber, offers and requests before creating an account.</p><div className="m-source-cta-proof"><span>No account needed to explore</span><span>Current offers</span><span>Trading-desk expertise</span></div><AppButtons/></div><Image className="m-cta-x" src="/images/fpx-logo-x-original.png" alt="" width={495} height={504} sizes="(max-width: 700px) 45vw, 32vw" quality={80}/></div></section>}
