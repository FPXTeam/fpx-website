    ...
    <section className="pri-library" aria-live="polite">
      {featured&&<>
        ...
        <Link href={featured.href} className="pri-featured">
          <div className={`pri-featured-image ${featured.image==="radiata-pine-characteristics-cover.png"?"is-contain":""}`}><Image src={`/images/${featured.image}`} alt={featured.imageAlt} width={1400} height={1000} sizes="(max-width: 1000px) 100vw, 52vw" priority/></div>
        ...
        <div className="pri-more">
          ...
          <div className="pri-grid">
            {remaining.map((item,i)=><Link href={item.href} className="pri-card" key={item.title}>
              <div className="pri-card-image"><Image src={`/images/${item.image}`} alt={item.imageAlt} width={1200} height={800} sizes="(max-width: 700px) 100vw, 50vw"/></div>
...
function RadiataPineArticle(){
  ...
  return <article className="ria">
    ...
    <section className="ria-sources">
      ...
      <ol>{sources.map(source=><li key={source.label}>{source.href?<a href={source.href} target="_blank" rel="noreferrer">{source.label}</a>:source.label}</li>)}</ol>
    </section>

    <section className="ria-next">
      <div><Eyebrow>NEXT IN INDUSTRY INSIGHTS</Eyebrow><h2>The Science of Kiln Drying</h2><p>Conventional and continuous kilns, moisture content, and why it matters for Radiata Pine.</p><nav className="ria-related" aria-label="Related FPX timber pages"><Link href="/timber">Explore timber range</Link><Link href="/manufacturing">Manufacturing timber</Link><Link href="/fpx-sourcing">FPX Sourcing</Link></nav></div>
